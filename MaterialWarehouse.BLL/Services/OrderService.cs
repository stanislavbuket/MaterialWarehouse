...
{
    public async Task<Result<OrderDto>> CreateOrderAsync(int userId, IEnumerable<(int materialId, int quantity)> items)
    {
        ...
        foreach (var (materialId, quantity) in items)
        {
            ...
            var orderItem = new OrderItem(orderItemId++, order.Id, materialId, quantity, isPreOrder);
            order.AddItem(orderItem);
            totalItems += quantity;
        }

        await unitOfWork.SaveChangesAsync();
        return Result.Success(new OrderDto(order.Id, order.UserId, order.State.ToString(), order.CreatedAt, totalItems));
    }

    public async Task<Result> TransitionOrderStateAsync(int orderId, string nextState)
    {
        ...
        var allowedCurrent = next switch
        {
            ...
            OrderState.Cancelled => new[] { OrderState.Placed, OrderState.Approved, OrderState.Shipped },
